...
def auto_imagen(imagen, width=96, height=72, border_radius=8) -> ft.Control:
    ...
    Mismo helper para miniaturas (en listas) y para la imagen ampliada del
    panel de detalle: solo cambian width/height.
    """
    if imagen:
        return ft.Image(
            src=imagen,
    ...
    return ft.Container(
        content=ft.Icon(ft.Icons.DIRECTIONS_CAR, color=Colors.TEXT_MUTED, size=min(28, width // 3)),
        width=width,
        height=height,
...
def top_bar(usuario_actual, active_tab: str, on_nav_click=None, on_account_click=None,
            on_search=None, valor_busqueda="") -> ft.Container:
    tabs = []
    ...
    header_row = ft.Row(
        [
            ...
            # Cuenta: avatar + nombre + flecha. Un solo clic abre el mini panel
            # de manejo de cuentas (ver views/account_panel.py), no navega
            # directo a ningún lado por sí mismo.
            ft.Container(
                content=ft.Row(
                    [
...
def page_shell(usuario_actual, active_tab, body, on_nav_click=None, on_account_click=None,
               on_search=None, valor_busqueda="") -> ft.Container:
