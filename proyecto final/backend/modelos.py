...
class Usuario:
    ...
    def __init__(self, id_usuario, nombre, email, password_hash, rol,
                 telefono="", foto_perfil=None, verificado=False, autos_en_posesion=None,
        ...
        # Foto de perfil: mismo criterio que Carro.imagen (ver más abajo) —
        # puede ser una URL o un string base64 (sin el prefijo 'data:'), o
        # None si el usuario todavía no puso ninguna.
...
class Carro:
    ...
    entrega_confirmada / fecha_entrega_confirmada: una vez que estado_subasta
    es 'vendido', el COMPRADOR puede marcar que ya recibió el vehículo físico
    (ver AdministradorCompraVenta.confirmar_entrega en sistema.py). Es un
    campo aparte de estado_subasta a propósito: la subasta ya está cerrada
    ('vendido') independientemente de si el auto ya cambió de manos en la
    vida real o no — son dos cosas distintas que no hay que mezclar en el
    mismo estado.
    """
    ...
    def __init__(self, id_carro, vendedor_id, marca, modelo, anio, kilometraje,
        ...
        # --- Confirmación de entrega física (post-venta), ver docstring ---
        self.entrega_confirmada = entrega_confirmada
        self.fecha_entrega_confirmada = fecha_entrega_confirmada
    ...
    def to_dict(self):
        ...
        return {
            ...
            "entrega_confirmada": self.entrega_confirmada,
            "fecha_entrega_confirmada": self.fecha_entrega_confirmada,
        }
...
class Mensaje:
    """
    Un mensaje de chat entre dos usuarios sobre un carro en particular.
    La 'conversación' queda definida por la combinación (id_carro, los dos
    ids de usuario) — así, si dos postores distintos le escriben al mismo
    vendedor sobre el mismo carro, no se mezclan en un solo hilo.
    """

    def __init__(self, id_mensaje, id_carro, id_remitente, id_destinatario,
                 texto, fecha_hora=None, leido=False):
        self.id = id_mensaje
        self.id_carro = id_carro
