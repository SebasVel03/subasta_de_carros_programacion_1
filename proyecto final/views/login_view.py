...
pero con diferencias necesarias para que funcione con el backend:
...
3. Ya NO hay selector de rol ni campo de código de administrador. El rol
   se decide solo, del lado del backend, según el formato del correo (ver
   sistema.registrar_usuario / PATRON_EMAIL_ADMIN en backend/sistema.py):
   un correo tipo nombre.admin.XX@dominio se registra como admin/experto
   automáticamente, siempre que todavía haya cupo. Esta vista ni siquiera
   sabe que esa lógica existe — solo manda nombre/correo/contraseña y
   muestra el error si sistema.registrar_usuario() lo rechaza (por
   ejemplo, si el cupo de administradores ya está lleno).
    ...
    def handle_submit(e):
        ...
        if estado["modo"] == "registro":
            ...
            if not nombre:
                ...
                page.update()
                return
            # El rol (usuario / admin) ya no se elige acá: sistema.py lo
            # decide solo según el formato del correo (ver PATRON_EMAIL_ADMIN
            # y MAX_ADMINISTRADORES en backend/sistema.py).
            ok, resultado = sistema.registrar_usuario(nombre, email, password)
        else:
